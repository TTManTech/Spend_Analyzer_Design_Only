...
| File | What |
| --- | --- |
| `index.html` | The page: logo, title, subtitle, month/year picker, YTD bubble, three rate bubbles and the spend-by-category donut (Spend, Investment, Savings) |
...
**Spend by category** (below the rate bubbles) is a donut of the selected month's or YTD year's spend per Category of Spend, net of refunds. Clicking a slice or its row in the list shows that category's top 7 purchases with the amount and its share of the category. Each category keeps a fixed colour; "Other" is hatched grey and "Services" is hatched green. The breakdowns are precomputed into `data/spend-rates.json` under each month's and year's `categories`.
